#!/usr/bin/env bash
# BikeCom — Xcode 프로젝트 생성 + Watch 타깃 확인 + Xcode 열기
set -euo pipefail
...
echo "==> 1/3 Xcode 프로젝트 생성 (project.yml → BikeCom.xcodeproj)"
xcodegen generate

echo "==> 2/3 Watch 타깃 확인"
if ! "$XCODEBUILD" -project BikeCom.xcodeproj -list 2>/dev/null | grep -q BikeComWatch; then
  echo "❌ BikeComWatch 타깃이 없습니다. project.yml 을 확인하세요."
  exit 1
fi

if ! grep -q 'BikeComWatch' BikeCom.xcodeproj/xcshareddata/xcschemes/BikeCom.xcscheme; then
  echo "❌ BikeCom 스킴에 Watch 빌드가 없습니다."
  exit 1
fi

echo "   ✓ 타깃: BikeCom, BikeComWatch"
echo "   ✓ 스킴: BikeCom (Watch 포함 빌드)"
...
echo "==> 4/4 Xcode 열기"
open BikeCom.xcodeproj

cat <<'EOF'

다음 단계 (Xcode):
  1. 상단 스킴: BikeCom  (BikeComWatch 아님)
  2. 기기: 본인 iPhone (시뮬레이터 X)
  3. 왼쪽 파란 아이콘 BikeCom → TARGETS:
       BikeCom        ← iPhone
       BikeComWatch   ← Watch (둘 다 있어야 함)
